...
def main():
    ...
    file_span_params = create_input_samples(args, combination_dir, compound_dir, gap_dir,
                                                  test_set_location)
...
def run_samples(combination_dir: Path, compound_dir: Path, gap_dir: Path, crops: FrozenSet[FOCUSCrop],
    ...
    pool = ThreadPoolExecutor(thread_name_prefix="orchestrators")
    if args.run == 'bhpc':
        bhpc = BHPC()
        logger.info("Starting deployment to BHPC")
        if args.run_test_set:
            logger.info("Starting to run test set on BHPC")
            pool.submit(run_bhpc, combination_dir=test_set_location, submit=args.work_dir / 'remote' / "test_data",
        ...
        pool.submit(run_bhpc, compound_file=compound_dir, gap_file=gap_dir, combination_dir=combination_dir,
...
def create_input_samples(args, combination_dir, compound_dir, gap_dir, test_set_location) -> Dict[str, list[str]]:
...
def create_samples_in_dirs(definition: Dict, output_dir: Path, sample_size: int,
    ...
    if cpus is None:
        cpus = 2
    pool = ThreadPoolExecutor(max_workers=cpus - 1, thread_name_prefix="create_samples")
...
def make_single_sample(definition: Definition, test_set: Set[Tuple[float, ...]], output_dir: Path,
    ...
    if not test_set:
        ...
        while True:
            ...
            distance = _shortest_distance_to_set(current_vector, test_set)
            if distance > test_set_buffer:
                combination = Combination(**combination_dict)
                with (output_dir / f"{hash(combination)}.json").open('w') as fp:
                    json.dump(combination, fp, cls=EnhancedJSONEncoder)
                break
            else:
                logging.getLogger().debug("Discarded sample for being to close to the test set. Sample was %s", combination_dict)
