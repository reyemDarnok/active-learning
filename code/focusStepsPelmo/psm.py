...
def main():
    ...
    results = run_psms(psm_files=psm_dir.glob('*.psm'), working_dir=focus_dir, focus_data=args.focus, crops=args.crop, scenarios=args.scenario, pelmo_exe=args.pelmo_exe, max_workers=args.threads)
...
def parse_args() -> Namespace:
    ...
    parser.add_argument('-e', '--pelmo-exe', type=Path, default=Path(__file__).parent / 'PELMO500.exe', help="The PELMO executable to use for running. Defaults to a bundled PELMO installation. This should point to the CLI EXE, usually named PELMO500.EXE NOT to the GUI EXE usually named wpelmo.exe.")
    ...
    parser.add_argument('-f', '--focus', type=Path, default=Path(__file__).parent / 'FOCUS.zip', help="The Focus data to use. Unzips it if it is a zip. Defaults to a bundled zip")
